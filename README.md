...
- **Subagents inherit the remap, and fableplan can't fix it.** A subagent set to `opus` runs **Fable**, at twice Opus's rate. Two routes reach it and fableplan controls neither: `CLAUDE_CODE_SUBAGENT_MODEL` in `settings.json`, whose value beats anything exported into the environment, and a subagent's own `model:` frontmatter, which never reads the environment at all — plugins ship plenty of agents pinned to `opus`. Pin full model names in `settings.json` if the difference matters to you.
