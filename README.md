...
- **Subagents inherit the remap, and fableplan can't fix it.** A subagent set to `opus` runs **Fable**, at twice Opus's rate. Both routes do it — `CLAUDE_CODE_SUBAGENT_MODEL=opus` and a subagent's own `model: opus` frontmatter — so no env var fableplan sets can catch the second one. Pin a full model name wherever you choose a subagent model, or expect Fable.
