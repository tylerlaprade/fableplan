...
- **Inside a fableplan session, `opus` means Fable and `sonnet` means Opus 5** — everywhere an alias is read, including fallback chains. The `/model` picker is honest (a **Fable Plan** entry marks the mode), but the startup banner still says "Opus Plan" (hardcoded in Claude Code).
- **Subagents inherit the remap.** A subagent set to `opus` runs **Fable**, at twice Opus's rate. Claude checks `CLAUDE_CODE_SUBAGENT_MODEL` before per-call and frontmatter choices, so a full model name there also controls plugin agents. Fableplan can't safely translate a user's alias for one run: in Claude Code 2.1.220, a user `settings.json` value beats both a process environment override and the same key passed through `--settings`, contrary to the documented CLI precedence ([upstream bug](https://github.com/anthropics/claude-code/issues/78567#issuecomment-5109786270)). Excluding user settings makes the override work but also drops the user's hooks and permission defaults. Pin a full model name in `settings.json` if the difference matters to you.
